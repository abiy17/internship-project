    ...
    return ( 
        <motion.div  id={mode ? "homePage" : "homePage-light"} className="flex items-center justify-center min-h-[42.46em">
            <div className="flex absolute top-5 left-10">
                <img className="sm:w-36 rounded-lg md:block w-14" src={mintImage} alt="" />
            </div>
            <div  className="relative left-[-1.2em] sm:relative sm:left-0 md:mt-48 lg:mt-0">
            ...
            <p className={mode ? "text-gray-300 text-2xl sm:mt-4 text-center pb-6" : "text-slate-800 sm:mt-4 text-2xl text-center pb-6"}>Direction indicator</p>
            <div  className="flex flex-col justify-center items-center relative left-16 w-[10em] m-auto">
                <motion.div initial={{opacity:1}} animate={{opacity:1}} exit={{opacity:0}} className="md:flex gap-[3em] pt-4 relative left-[-3em] pb-24">
                    <NavLink to="personal"><button className={mode ? "button h-[4em]" : "button-light h-[4em]"}><span className={mode ? "flex pl-3 flex-col" : "flex pl-2 flex-col"}>የስራ ሐላፊዎች  Officials</span></button></NavLink>
