    ...
    return (
        <motion.div className={mode ?"Directions" : "Directions-light"}>
            <div className={mode ? "DepartmentNav sticky top-3 flex h-[3em] rounded justify-around gap-24 w-11/12 m-auto" : "DepartmentNav-light sticky top-3 flex h-[3em] rounded justify-around gap-24 w-11/12 m-auto"}>
                <h1 className={mode ? "text-white flex gap-2 font-serif items-center" : "text-slate-800 flex gap-2 items-center font-serif"}><FaBuilding /><p className="hidden sm:block">Departments</p></h1>
                <button onClick={()=>Navigate(-2)} className={mode ? "text-white hover:opacity-40 duration-300 flex font-serif items-center" : "text-slate-800 hover:opacity-40 duration-300 font-serif flex items-center"}><FaMagento  className="mr-3"/><p className="hidden sm:block">Main Menu</p></button>
            </div>
            <Rating />
            <div className="flex gap-4 w-11/12 m-auto mt-7">
                <div className={mode ? "DepartmentNavv hidden md:block scroll-smooth overflow-y-scroll scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-300 rounded-xl w-[35%] h-[36em]" : "DepartmentNavv-light hidden md:block overflow-y-scroll scroll-smooth scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-300 rounded-xl w-[35%] h-[36em]"}>
                    <h1 className={mode ? "text-white relative top-[-3em] text-[2em]" : "text-slate-800 relative top-[-3em] font-serif text-[2em]"}>Other Departments</h1>
